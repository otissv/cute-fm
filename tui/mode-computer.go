...
func (m Model) ComputerMode(msg tea.Msg) (tea.Model, tea.Cmd) {
	var (
		cmd  tea.Cmd
		cmds []tea.Cmd
	)
	...
	// Update the computer list to handle scrolling
	m.computerList, cmd = m.computerList.Update(msg)
	...
	// to the next navigation command in the computer list.
	//
	...
	// Move cursor down in computer list (with optional count)
	case bindings.CursorDown.Matches(key):
		for i := 0; i < count; i++ {
			m.computerList.CursorDown()
		}
		return m, tea.Batch(cmds...)

	// Move cursor up in computer list (with optional count)
	case bindings.CursorUp.Matches(key):
		for i := 0; i < count; i++ {
			m.computerList.CursorUp()
		}
		return m, tea.Batch(cmds...)
